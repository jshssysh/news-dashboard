...
# "인사" 카테고리에서만 인사 발령/부고를 다루는 8개 기관(삼성 제외 - 삼성 인사는
# 삼성그룹 카테고리로 별도 추적됨). Gemini 분류 프롬프트에서 relevance_score 게이트를
# 우회시킬 대상을 명시하는 데 쓴다.
PERSONNEL_NEWS_ORGS = [
    "공정거래위원회", "국토교통부", "환경부", "고용노동부", "중소벤처기업부",
...
4. summary: (relevance_score 5점 이상일 때만) 1문장 핵심 요약.
   문장 종결은 "~합니다/~했습니다" 같은 정중체 대신 "~함/~했음/~임"처럼 간결한
   개조식으로 끝낼 것. 예(좋음): "리니언시 적용 두고 형평성 논란 제기됨".
   예(나쁨): "리니언시 적용을 두고 형평성 논란이 제기되고 있습니다".
...
6. category: relevance_score와 무관하게 모든 기사에 대해 판정. 아래 후보 중 기사 내용에 가장 적합한 것 정확히 하나만 선택.
   ...
   "인사"는 다음 8개 기관({", ".join(PERSONNEL_NEWS_ORGS)})의 임원급 이상 인사 발령 또는
   부고(별세) 안내 기사에만 쓸 것 - 삼성 관련 인사·부고는 "인사"가 아니라 삼성그룹/삼성물산으로 분류.
7. category가 "인사"로 판정된 기사는 위 2~5번 규칙 대신 다음을 그대로 따를 것(인사 동정은
   업황과 무관해 위 규칙으로 평가할 대상이 아님):
   - relevance_score: 항상 5 (대시보드에 노출은 되지만, 각종 "중요도순" 정렬·"오늘 주요
     내용" 하이라이트에서는 일반 이슈 기사보다 낮게 밀리도록 일부러 낮게 고정하는 값임 -
     8 등 높은 값을 쓰지 말 것)
   - sentiment: 항상 "중립"
   - group_title: 인물명을 반드시 포함해 "{{인물명}} 인사" 또는 "{{인물명}} 부고" 형식
   - summary: 인사 발령이면 소속·직책 변경 내용을, 부고면 고인과의 관계(본인/부친상/모친상 등)를
     1문장으로 정리
...
GENERIC_ISSUE_TITLES_BLOCKLIST = {
    ...
    # Opus 리뷰(2026-09-18)로 찾은 오염 비율이 매우 높은(90%대) 포괄 바구니 -
    # "상생협력"·"동반성장 지원"·"M&A·인수합병" 한 단어짜리 이슈명이 서로 완전히
    # 무관한 기업의 기사를 계속 재사용해온 것으로 확인됨. "공정위조사"도 마찬가지로
    # 회사명 없이 사건유형만 있어 재사용 후보에서 빼야 함.
    "상생협력", "공정위조사", "동반성장 지원", "M&A·인수합병",
}
...
CASE_TYPE_GENERIC_WORDS = {
    ...
    # 입법 관련 이슈명도 같은 문제가 실측됨(2026-09-18): "이언주 의원 법안 발의"라는
    # 기존 이슈가 "의원"·"법안"·"발의"만 겹친다는 이유로 이강일·오기형·강준현·송옥주
    # 등 서로 완전히 무관한 의원의 개별 법안 발의 기사를 전부 흡수해버림 - 위
    # 사건유형어처럼 이런 입법 절차 boilerplate 단어도 "구체적으로 겹치는 단어"로
    # 치면 안 된다.
