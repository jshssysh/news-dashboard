...
뉴스와 달리 새 의결서/판례는 일주일에 많아야 수십 건이라, daily.yml처럼 하루
5번씩 돌 필요가 없다 - main()이 토요일 06시대 실행일 때만 실제로 수집하고
나머지 실행은 바로 종료한다(자세한 이유는 main() 안 주석 참고).
...
# "원심결"/"원심판결"이라는 라벨 단어(원문 서식상 흔히 "【원심판결】"처럼 대괄호가
# 붙는다) 자체는 화면에 보여줄 필요가 없다 - 그 라벨이 있다는 사실로 진짜 원심
# 언급인지만 확인(매칭 앵커)하고, 실제로 보여주는 건 그 뒤에 오는 법원명/날짜/
# 사건번호 부분만이다(캡처 그룹으로 분리).
...
def main():
    # 의결서/판례는 뉴스와 달리 일주일에 많아야 수십 건이라, daily.yml처럼 하루
    # 5번 다 돌 필요가 없다 - 주 1회, 토요일 06:01 실행 때만 실제로 수집하고
    # 나머지 4번(같은 날 07:12/07:47/12:01/18:01)과 평일은 그냥 건너뛴다.
