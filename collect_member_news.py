...
보고 싶다는 요청이라 사람당 상한 없이 전부 누적한다(날짜로 잘라내지 않음
- news_list.csv는 30일 지나면 자동으로 빠지고, bill_list.csv는 애초에
누적이 아니라 매번 국회 API가 주는 전체 목록으로 통째로 새로 쓰는 방식이라
이 파일과는 방식이 다르다). 하루 증분만 새로 API를 태우는 것은 셋 다 같다.
...
# "오늘의 국회일정", 날짜만 있는 제목("2026년 9월 1일", "(1일, 화)"), 특정 언론사의
# 반복 칼럼("국회의장·상임위·세미나·기자회견·주요 정당 -9월...") 같은 캘린더성 항목.
# 여러 의원 이름이 한 목록에 나열될 뿐 특정 사실을 담은 기사가 아닌데, Gemini가
# 그 의원에 대해 알고 있는 엉뚱한 사실을 갖다 붙이는 사고가 실측돼서(예: "(1일, 화)"라는
# 제목에 "강원FC 원정응원단 예산 위법성 기자회견"처럼 구체적인 요약이 붙음) 애초에
# 후보에서 제외한다.
CALENDAR_TITLE_PATTERN = re.compile(
    r"오늘의\s*(국회)?\s*(주요)?\s*일정"
    r"|^\(?\d{1,2}[일]?[,·.]?\s*[월화수목금토일]\)?$"
    r"|^\d{4}년\s*\d{1,2}월\s*\d{1,2}일"
    r"|국회의장.{0,3}상임위.{0,3}세미나"
)
...
def main():
    ...
    for i, name in enumerate(names):
        ...
        for item in items:
            ...
            if CALENDAR_TITLE_PATTERN.search(title):
                continue  # 캘린더성 제목(오늘의 일정 등) - 특정 사실을 담은 기사가 아님
